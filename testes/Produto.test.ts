...
describe("Classe Produto", ()=>{
	...
	describe("Ao criar um produto", ()=>{
		test("Deve criar um produto válido com todas as propriedades e desconto fixo", () => {
            ...
            expect(p).toMatchObject({nome, estoque, desconto, precoAtual:precoOriginal-desconto})
        })			
        ...
        test("Deve criar um produto válido sem desconto", () => {
            const p = new ProdutoBuilder()
                            ...
                            .comPrecoOriginal(precoOriginal)
                            .build()
            expect(p).toMatchObject({nome, estoque, desconto:undefined, precoAtual:precoOriginal})
        })
	...
	describe("Ao mudar o desconto", ()=>{
		test("Deve aplicar o desconto fixo corretamente", ()=> {
            const p = new ProdutoBuilder().padrao().comPrecoOriginal(precoOriginal).build()
            const desconto = 10
            p.desconto = desconto
		...
		test("Deve lançar um erro ao tentar alterar o valor para negativo", ()=> {
            const p = new ProdutoBuilder().padrao().comPrecoOriginal(precoOriginal).build()
            const desconto = -10
            const alteraco = () => p.desconto = desconto
            expect(alteraco).toThrow("Desconto deve ser positivo")
        })

        test("Deve lançar um erro ao tentar alterar o valor para negativo casa decimal", ()=> {
            const p = new ProdutoBuilder().padrao().comPrecoOriginal(precoOriginal).build()
            const desconto = -0.5
            const alteraco = () => p.desconto = desconto
            expect(alteraco).toThrow("Desconto deve ser positivo")
	...
	describe("Ao obter a descrição", ()=>{
        const produtoParcial = () => new ProdutoBuilder()
                                        .comNome(nome)
                                        .comEstoque(estoque)
                                        .comPrecoOriginal(precoOriginal)
		test("Deve retornar a descrição de um produto com desconto fixo", () => {
            ...
            expect(descricao).toBe(`Produto: ${nome}, Preço: R$${(precoOriginal-desconto).toFixed(2)}, Desconto: R$${desconto.toFixed(2)}, Preço Original: R$${precoOriginal.toFixed(2)}`)            
        })
		test("Deve retornar a descrição de um produto com desconto percentual", () => {
            const desconto = 0.5
            const p = produtoParcial().comDesconto(desconto).build()
            const descricao = p.getDescricao()
            expect(descricao).toBe(`Produto: ${nome}, Preço: R$${(precoOriginal*(1-desconto)).toFixed(2)}, Desconto: ${desconto*100}%, Preço Original: R$${precoOriginal.toFixed(2)}`)            
        })
		test("Deve retornar a descrição de um produto sem desconto", () => {
            const p = produtoParcial().build()
            const descricao = p.getDescricao()
            expect(descricao).toBe(`Produto: ${nome}, Preço: R$${precoOriginal.toFixed(2)}`)
        })
